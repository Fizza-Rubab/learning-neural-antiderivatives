...
# def gaussian(sigma):
#     def kernel(x):
#         Z = 0.5 * (jax.scipy.special.erf(sigma / (sigma * jnp.sqrt(2))) - 
#                    jax.scipy.special.erf(-sigma / (sigma * jnp.sqrt(2))))
#         return (1. / (sigma * jnp.sqrt(2 * jnp.pi))) * jnp.exp(-(x) ** 2 / (2 * sigma ** 2)) / Z
#     return kernel

def gaussian(sigma):
    def kernel(x):
        # Effective support is [-sigma, sigma]
        a, b = -sigma, sigma
        Z = 0.5 * (erf(b / (jnp.sqrt(2) * sigma)) - erf(a / (jnp.sqrt(2) * sigma)))
        norm = sigma * jnp.sqrt(2 * jnp.pi) * Z
        return jnp.exp(-(x) ** 2 / (2 * sigma ** 2)) / norm
    return kernel
...
def minimal_kernel_diracs(order, s):
    if order == 0:
        ...
        dirac_y = np.array([-1., 3., -3., 1.]) * 0.5 * 6 / s**3
    else:
        assert False, "Only orders 0-2 implemented"
