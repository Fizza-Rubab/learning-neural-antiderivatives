...
def _parse_args():
    ...
    parser.add_argument("--pose", type=str, default=None, help="Monte_carlo gt path")
...
def _main():
    ...
    if args.blur == 1:
        monte_carlo_gt = load_montecarlo_gt(args.pose)
    else:
        if args.is_analytic == 1:
            monte_carlo_gt = np.load(args.pose)
        else:
            monte_carlo_gt = read_pose(args.pose)
            print("monte_carlo_gt", monte_carlo_gt.shape)
