...
from utilities import do_3d_gaussian_dv_conv
...
def _parse_args():
    ...
    parser.add_argument("--geometry", type=str, default=None, help="Monte_carlo gt path")
    ...
    parser.add_argument("--blur", type=int, default=0, help="whether or not we are using a blurred version")

    parser.add_argument("--is_analytic", type=int, default=0, help="seed value for the training")
...
def _main():
    ...
    if args.analytic.lower() in ["gm", "ackley", "hr"]:
        ...
        if args.blur == 1:
            monte_carlo_gt = load_montecarlo_gt(args.geometry)
        else:
            monte_carlo_gt = mesh_to_sdf_tensor(args.geometry, 256)
            monte_carlo_gt = pad_sdf(monte_carlo_gt, 0.3)
    ...
    if monte_carlo_gt is not None:
        ...
        if args.analytic.lower() == "ackley":
            ...
            def analytic_fn(xy):
                val = gm_pdf.eval(xy.detach().cpu().numpy())
            ...
            def analytic_fn(xy):
                val = hm.eval(xy.detach().cpu().numpy())
                return torch.from_numpy(val).to(xy.device).unsqueeze(-1).float()

            interpolator_fn = analytic_fn

    conv_fn = do_3d_gaussian_dv_conv
    print(f'function path')
    print(args.func_path)
    # do_3d_conv
