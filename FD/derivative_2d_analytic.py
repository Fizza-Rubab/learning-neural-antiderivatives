...
def evaluate_model(net_path, func_name, order, size=1024):
    ...
    print(f"MSE:   {mse:.10f}")
    print(f"PSNR:  {psnr:.10f}")
    print(f"SSIM:  {ssim:.10f}")
    print(f"LPIPS: {lpips_score:.10f}")
...
if __name__ == "__main__":
    for func_name in [ "hr"]:
        for order in [0, 1]:
            ...
            net_path = f"/HPS/antiderivative_project/work/NFC-Blur/experiments/results_2d/NFC-Blur_{func_name}_order={order}/current.pth"
