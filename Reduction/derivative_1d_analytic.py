...
def nth_derivative(model, x, order):
    if order == 1:
        ...
        y = vmap(jacrev(jacfwd(lambda a: a*model(a)[:1] - model(a)[1:2])))(x).reshape(-1, 1)
    elif order == 3:
        y = vmap(jacfwd(jacrev(jacfwd(lambda a: 0.5*a**2*model(a)[0:1] - a*model(a)[1:2] + 0.5*model(a)[2:3]))))(x).reshape(-1, 1)
    return y
...
def evaluate_all():
    root_model_dir = "/HPS/antiderivative_project/work/Reduction/experiments/results_1d"
    ...
    for func in functions:
        for order in orders:
            ...
            model_path = os.path.join(root_model_dir, f"Reduction_{func}_order={order}", "current.pth")
