...
def nth_derivative(model, x, order):
    if order == 1:
        ...
        y = vmap(jacrev(jacfwd(lambda a: a*model(a)[:69] - model(a)[69:])))(x).reshape(-1, 1)
    elif order == 3:
        y = vmap(jacfwd(jacrev(jacfwd(lambda a: 0.5*a**2*model(a)[0:69] - a*model(a)[69:2*69] + 0.5*model(a)[2*69:]))))(x).reshape(-1, 1)
    return y
...
def main():
    ...
    ckpt_root = "/HPS/antiderivative_project/work/Reduction/experiments/results_1d"
