...
def nth_derivative(model, x, order):
    if order == 1:
        return vmap(jacfwd(jacrev(jacfwd(lambda a, b, c:
            model(torch.cat([a, b, c], -1)), argnums=0), argnums=1), argnums=2))(
            x[:, 0:1], x[:, 1:2], x[:, 2:3]
        ).reshape(-1, 1)

    elif order == 2:
        def autoint_second_order(a, b, c):
        ...
        return vmap(
            jacfwd(jacrev(jacfwd(jacfwd(jacrev(jacfwd(autoint_second_order, argnums=0), argnums=1),argnums=2), argnums=0), argnums=1), argnums=2))(x[:, 0:1], x[:, 1:2], x[:, 2:3]).reshape(-1, 1)
...
def main():
    ...
    ckpt_root = "/HPS/antiderivative_project/work/Reduction/experiments/results_3d"
