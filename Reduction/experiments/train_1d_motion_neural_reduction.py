...
def main():
    ...
    dim = 1
    out_channels = 69*args.order**dim
    ...
    for step in range(args.num_steps):
        ...
        if args.order==1:
            ...
            f_derivative = derivative[:, :69] 
            loss_f = F.smooth_l1_loss(ground_truth_f, f_derivative)
            loss = loss_f
        elif args.order==2:
            ground_truth_fx = model_input * ground_truth_f
            derivative = vmap(jacfwd(lambda a: model(a)))(model_input).reshape(-1, 69*2)
            f_derivative = derivative[:, :69]
            fx_derivative = derivative[:, 69:]
            loss_f = F.smooth_l1_loss(ground_truth_f, f_derivative)
            ...
            derivative = vmap(jacfwd(lambda a: model(a)))(model_input).reshape(-1, 69*3)
            f_derivative = derivative[:, :69]
            fx_derivative = derivative[:, 69:69*2]
            fx2_derivative = derivative[:, 69*2:]
            loss_f = F.smooth_l1_loss(ground_truth_f, f_derivative)
            loss_fx = F.smooth_l1_loss(ground_truth_fx, fx_derivative)
