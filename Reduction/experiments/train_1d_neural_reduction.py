...
def pad_signal_1d(signal, pad_fraction=0.3):
    ...
    padded_signal = np.pad(signal, ((pad_length, pad_length), (0, 0)) if len(signal.shape) > 1 else (pad_length, pad_length), mode='reflect')
...
def _parse_args():
    ...
    parser.add_argument("--audio", type=str, default=None, help="Audio npy path")
...
def main():
    ...
    for step in range(args.num_steps):
        ...
        if args.order==1:
            ...
            ground_truth_fx = model_input * ground_truth_f
            derivative = vmap(jacfwd(lambda a: model(a)))(model_input).reshape(-1, 2)
            f_derivative = derivative[:, :1]
            fx_derivative = derivative[:, 1:]
            loss_f = F.smooth_l1_loss(ground_truth_f, f_derivative)
            loss_fx = F.smooth_l1_loss(ground_truth_fx, fx_derivative)
            loss = loss_f + loss_fx
        elif args.order==3:
            ground_truth_fx = model_input * ground_truth_f
            ground_truth_fx2 = model_input**2 * ground_truth_f
            derivative = vmap(jacfwd(lambda a: model(a)))(model_input).reshape(-1, 3)
            f_derivative = derivative[:, :1]
        ...
        if step % steps_till_summary == 0:
            if args.order==1:
                ...
                print(f'Iteration: {step}, Loss: {loss.item():.9f}, f: {loss_f.item():.9f}, fx: {loss_fx.item():.9f}', flush=True)
                writer.add_scalar('loss/f', loss_f.item(), step)
                writer.add_scalar('loss/fx', loss_fx.item(), step)
                writer.add_scalar('loss/total', loss.item(), step)
            elif args.order==3:
                print(f'Iteration: {step}, Loss: {loss.item():.9f}, f: {loss_f.item():.9f}, fx: {loss_fx.item():.9f}, fx2: {loss_fx2.item():.9f}', flush=True)
                writer.add_scalar('loss/f', loss_f.item(), step)
                writer.add_scalar('loss/fx', loss_fx.item(), step)
