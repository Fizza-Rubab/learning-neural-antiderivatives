...
def mesh_to_sdf_tensor_eval(mesh_path, resolution):
    def scale_to_unit_cube(mesh):
        if isinstance(mesh, trimesh.Scene):
            mesh = mesh.dump().sum()
    ...
    # Rearranging the ground truth to match (X, Y, Z) order
    sdf_tensor = np.transpose(sdf_tensor, (2, 1, 0))  # Reorder to (X, Y, Z)
    return sdf_tensor
...
def mc_conv_2d(path,
    ...
    mc = mc_convolution(image,
                        ...
                        min1 if kernel_type=="minimal" else gaussian,
                        2,
                        image.shape[0])
