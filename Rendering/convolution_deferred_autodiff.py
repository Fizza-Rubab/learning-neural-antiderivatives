...
class SceneOP(OpenGLOP):
    def __init__(self, resolution):
        ...
        self.shininess = 64
        self.order = 1
        os.makedirs(rf"results\Autoint\order{self.order}", exist_ok=True)
        ...
        model_path = rf"..\models\envmodels\Autoint\Autoint_dikhololo_night_1k_order={self.order+1}.pth"
    ...
    def render(self, camera):
        ...
        save_path = os.path.join(rf"results\Autoint\order{self.order}", f"{self.model_name}_{self.shininess}_{self.mesh_name}_view{self.view_idx}.exr")
