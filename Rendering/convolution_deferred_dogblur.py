...
class SceneOP(OpenGLOP):
    def __init__(self, resolution):
        ...
        self.shininess = 64
        self.order = 1
        os.makedirs(rf"results\DoG-Blur\order{self.order}", exist_ok=True)
        ...
        if self.order==0:
            ...
            self.weight_scale = 1.4 * (np.pi**2) ** (self.order + 1) * 0.01**4
        ...
        model_path = rf"..\models\envmodels\NFC-DoG-Blur\DoG_Blur_large_corridor_1k_2d_order_{0}_gaussian_0.03_samples_100000_2d_scale_0.01_order_{self.order}.pth"
    ...
    def render(self, camera):
        ...
        save_path = os.path.join(rf"results\DoG-Blur\order{self.order}", f"{self.model_name}_{self.shininess}_{self.mesh_name}_view{self.view_idx}.exr")
