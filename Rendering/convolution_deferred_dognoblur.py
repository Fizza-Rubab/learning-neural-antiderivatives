...
class SceneOP(OpenGLOP):
    def __init__(self, resolution):
        ...
        mesh_path = "meshes/Stanford_armadillo.ply"
        ...
        self.shininess = 32
        self.order = 1
        os.makedirs(rf"results\DoG-Noblur\order{self.order}", exist_ok=True)
        ...
        # 1.1 calibration
        if self.order==0:
            self.weight_scale = (np.pi**2) ** (self.order + 1) * 0.01**2
        else:
            self.weight_scale = 1.4 * (np.pi**2) ** (self.order + 1) * 0.01**2
        ...
        # dikhololo_night_1k.exr
        model_path = rf"..\models\envmodels\NFC-DoG-Noblur\DoG_Noblur_meadow_1k_2d_scale_0.01_order_{self.order}.pth"
    ...
    def render(self, camera):
        ...
        with torch.no_grad():
            preds = evaluate_network_in_chunks(self.img_siren_f, uv_input_switched, chunk_size=1000_000).view(self.num_diracs, -1, 3)
        ...
        save_path = os.path.join(rf"results\DoG-Noblur\order{self.order}", f"{self.model_name}_{self.shininess}_{self.mesh_name}_view{self.view_idx}.exr")
