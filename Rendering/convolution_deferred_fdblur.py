...
class SceneOP(OpenGLOP):
    def __init__(self, resolution):
        ...
        self.shininess = 64
        self.order = 1
        os.makedirs(rf"results\NFC-Blur\order{self.order}", exist_ok=True)
        ...
        model_path = rf"..\models\envmodels\NFC-Blur\NFC-Blur_dikhololo_night_1k_2d_order_{self.order}_minimal_0.04_samples_200000_order={self.order}.pth"
    ...
    def render(self, camera):
        ...
        save_path = os.path.join(rf"results\NFC-Blur\order{self.order}", f"{self.model_name}_{self.shininess}_{self.mesh_name}_view{self.view_idx}.exr")
