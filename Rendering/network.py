...
def get_img_grid(shape):
    coords_y = np.linspace(0, 1, shape[0], endpoint=False)
    coords_x = np.linspace(0, 1, shape[1], endpoint=False)
    xy_grid = np.stack(np.meshgrid(coords_y, coords_x), -1)  # Swap coords_x and coords_y
    xy_grid = torch.tensor(xy_grid).float().contiguous().reshape(-1, 2).to('cuda')
    return xy_grid

def pad_image(image, pad_fraction=0.2):
    H, W, C = image.shape
    pad_h = int(H * pad_fraction)
    pad_w = int(W * pad_fraction)
    left_pad = image[:, -pad_w:, :]
    right_pad = image[:, :pad_w, :]
    image_horiz_padded = np.concatenate([left_pad, image, right_pad], axis=1)
    top_pad = np.flip(image_horiz_padded[:pad_h, :, :], axis=0)
    bottom_pad = np.flip(image_horiz_padded[-pad_h:, :, :], axis=0)
    padded_image = np.concatenate([top_pad, image_horiz_padded, bottom_pad], axis=0)
    return padded_image
...
def scale_image_pytorch(image):
    min_value = torch.min(image)
    max_value = torch.max(image)
    scaled_image = (image - min_value) / (max_value - min_value)
    return scaled_image

def evaluate_network_in_chunks(network, uv_flat, chunk_size=500_000):
    outputs = []
    for i in range(0, uv_flat.shape[0], chunk_size):
        chunk = uv_flat[i:i+chunk_size]
        with torch.no_grad():
            out = network(chunk)
            out = out[:]
        outputs.append(out) 
...
def evaluate_network_in_chunks_red(network, uv_flat, chunk_size=500_000):
    outputs = []
    for i in range(0, uv_flat.shape[0], chunk_size):
        chunk = uv_flat[i:i+chunk_size]
        with torch.no_grad():
